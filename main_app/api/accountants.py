...
never-editable generated ID.

Note who is allowed in here: creating and removing the accounts office is an
ADMIN job, and everything the office then does with money is not. That split
is the whole reason the role exists — see the Accountant model's docstring.
"""
...
def accountant_list(request):
    ...
    try:
        ...
        data['detail'] = ("Accountant account created, but the verification "
                          ...
                          "email\" from the accountant list once email "
                          "delivery is fixed.")
...
def accountant_item(request, accountant_id):
    ...
    if request.method == 'DELETE':
        # PROTECT on every financial FK means this refuses outright once the
        # accountant has issued an invoice or taken a payment — deliberately.
        # Their name is on receipts; the account has to stay to explain them.
        try:
            accountant.admin.delete()
        except Exception:
            return Response(
                {'detail': "This accountant has money on the record — "
                           "invoices raised or payments taken — so the "
                           "account can't be removed without orphaning those "
                           "receipts."},
                status=status.HTTP_400_BAD_REQUEST)
