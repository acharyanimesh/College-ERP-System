...
function ManageAccountant() {
  ...
  const remove = async (accountant) => {
    ...
    try {
      ...
      // The server refuses once this accountant's name is on a receipt — say
      // which it is rather than a flat "could not delete".
      addMessage(
        err.response?.data?.detail || "Could not delete the accountant.",
        "danger"
      );
    }
  };
